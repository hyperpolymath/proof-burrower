...
find "$TEST_REPO_PATH" -type f \
    ...
    -exec bash -c '
        file="$1"
        placeholder_pairs=(
            "proof-burrower|$TEST_REPO_NAME"
            "hyperpolymath|$TEST_OWNER"
            "github.com|$TEST_FORGE"
            "Proof Burrower|$TEST_PROJECT_NAME"
            "{{project}}|'"${TEST_REPO_NAME//-/_}"'"
            "Find the mathematical home for a novel proof goal|$TEST_DESCRIPTION"
            "Rust|$TEST_PRIMARY_LANGUAGE"
            "Jonathan D.A. Jewell|$TEST_AUTHOR"
            "j.d.a.jewell@open.ac.uk|$TEST_AUTHOR_EMAIL"
            "2026-04-26|2026-04-04"
        )
