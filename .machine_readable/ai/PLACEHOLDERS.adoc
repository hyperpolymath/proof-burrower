...
sed -i 's/Proof Burrower/my-project/g' $(grep -rl 'Proof Burrower' .)
sed -i 's/Proof Burrower/MY_PROJECT/g' $(grep -rl 'Proof Burrower' .)
...
sed -i "s/2026-04-26/$(date +%Y-%m-%d)/g" $(grep -rl '2026-04-26' .)
```
...
| `JJ` | Author initials (for citations) | `J.` | docs/CITATIONS.adoc |

### Project Identity

| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Proof Burrower` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `Find the mathematical home for a novel proof goal — corpus-similarity search across mathlibs` | One-line description | `A tool for X` | flake.nix |
| `Proof Burrower` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `proof-burrower` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `2026-04-26` | Current date (ISO) | `2026-02-14` | STATE.a2ml, MAINTAINERS.md |
| `2026-04-26` | Last updated date | `2026-02-14` | TOPOLOGY.md, THREAT-MODEL.md |
...
| `https://github.com/hyperpolymath/proof-burrower` | Project website | `https://example.org` | SECURITY.md |
...
| `Given a novel proof goal, find candidate mathematical neighbourhoods where the goal naturally fits.` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
