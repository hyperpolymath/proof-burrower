...
This document provides a denotational semantics for AffineScript,
interpreting programs as mathematical objects in suitable semantic
domains. We use domain theory for handling recursion, monads for
effects, and logical relations for establishing semantic properties.

=== 1. Introduction

Denotational semantics provides: - Compositional interpretation of
programs - Mathematical foundation for reasoning - Basis for program
equivalence - Connection to logic and category theory

=== 2. Semantic Domains

==== 2.1 Basic Domains

We use complete partial orders (CPOs) with least elements (pointed
CPOs):

*Definition 2.1 (Pointed CPO)*: A pointed CPO (D, ⊑, ⊥) consists of: - A
set D - A partial order ⊑ on D - A least element ⊥ ∈ D - Every ω-chain
...
The ownership system ensures safety statically; runtime behavior is
identical.
...
*Theorem 9.2 (Semantic Soundness)*: If `+Γ ⊢ e : τ+` then for all ρ ∈
⟦Γ⟧:
...
The meaning of a compound expression depends only on the meanings of its
parts.
...
*Theorem 10.1 (Fundamental Property)*: For all `+Γ ⊢ e : τ+` and related
environments ρ₁ R_Γ ρ₂:
...
Logical relations prove: - Parametricity (free theorems) - Termination -
...
[arabic]
. Scott, D. S. (1976). Data Types as Lattices. _SIAM J. Computing_.
...
. Moggi, E. (1991). Notions of Computation and Monads. _I&C_.
...
dependencies - Mechanized proof: See `+mechanized/coq/Denotational.v+`
(stub)
