...
// Replace rsr-template-repo, proof-burrower, {{DEPS}} with actuals
...
| `$PREFIX/share/proof-burrower/`
| Data files, assets

| `$PREFIX/share/doc/proof-burrower/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/proof-burrower/config.toml`

Fallback: `$HOME/.config/proof-burrower/config.toml`
...
Or via OPSM: `opsm update proof-burrower`
...
[source,bash]
----
just install --prefix=/opt/proof-burrower-instance1 --config=/etc/proof-burrower/instance1.toml
just install --prefix=/opt/proof-burrower-instance2 --config=/etc/proof-burrower/instance2.toml
----
